"""
Script to migrate media files from local storage to Backblaze B2.
Run this script with: python scripts/migrate_media_to_b2.py
"""
...
def upload_file_to_b2(bucket, file_path, object_name=None):
    ...
    try:
        ...
        logger.debug(f"Uploaded {file_path} to {object_name} with ID {uploaded_file.id_}")
        return True
    except Exception as e:
...
def migrate_media_files():
    """Migrate all media files from local storage to B2."""
    ...
    with tqdm(total=total_files, desc="Migrating files") as pbar:
        for root, _, files in os.walk(media_root):
            for file in files:
                ...
                # Upload file to B2
                if upload_file_to_b2(bucket, file_path, rel_path):
                    success_count += 1
                else:
                    error_count += 1
...
def test_b2_connection():
    """Test the connection to B2."""
    try:
        # Create B2 API client and get bucket
        _, bucket = create_b2_api()

        # List objects in bucket
        logger.info(f"Listing files in bucket {bucket.name}...")
        file_count = 0
        # B2 SDK v2 doesn't support limit parameter directly
        for file_info, _ in bucket.ls():
            file_count += 1
        ...
        if file_count > 0:
            logger.info(f"Successfully connected to B2 bucket {bucket.name}")
            logger.info(f"Found {file_count} objects in bucket (showing first 5)")
        else:
            logger.info(f"Successfully connected to B2 bucket {bucket.name} (empty bucket)")
...
if __name__ == "__main__":
    print("EduMore360 Media Migration to Backblaze B2")
    print("==========================================")

    # Test B2 connection
    print("\nTesting connection to B2...")
    if not test_b2_connection():
        print("Failed to connect to B2. Please check your credentials and try again.")
        sys.exit(1)

    # Confirm migration
    print("\nThis script will migrate all media files from your local storage to Backblaze B2.")
    print("Your local files will NOT be deleted.")
    print("Files will be stored in a private bucket with signed URLs.")
